...
#include <assembly_msgs/TaskSpaceMultipleMoveAction.h>

using namespace dyros_math;
class TaskSpaceMultipleMoveActionServer : public ActionServerBase
{
public:
  // Joint Trajectory Execution Server (Moveit Planning)
  actionlib::SimpleActionServer<assembly_msgs::TaskSpaceMultipleMoveAction> as_;

  assembly_msgs::TaskSpaceMultipleMoveFeedback feedback_;
  assembly_msgs::TaskSpaceMultipleMoveResult result_;
  assembly_msgs::TaskSpaceMultipleMoveGoalConstPtr goal_;
  ...
  std::map<std::string, Eigen::Isometry3d> target_poses_;

  TaskSpaceMultipleMoveActionServer(std::string name, ros::NodeHandle &nh, 
                          std::map<std::string, std::shared_ptr<FrankaModelUpdater> > &mu);
  // bool getTarget(ros::Time time, Eigen::Matrix<double, 7, 1> & torque) override; //command to robot
  bool compute(ros::Time time) override;
  bool computeArm(ros::Time time, const std::string &arm_name);
  ...
  std::map<std::string, SubTaskState> sub_tasks_1_;
  std::map<std::string, SubTaskState> sub_tasks_2_;
    // For convinient
